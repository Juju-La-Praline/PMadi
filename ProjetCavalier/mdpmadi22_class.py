...
class Display:
    def __init__(self, zoom, env, v=None) -> None:
        self.window = Tk()
            ...
            for j in range(self.env.nb_columns):
                ...
                if self.env.g[i, j] > 0:
                    ...
                    if self.env.g[i, j] < 0:
                        ...
                        self.Canvas.create_rectangle(x, y, x + self.zoom * 20, y + self.zoom * 20, fill=self.my_black)
        y =self.zoom*20*self.env.iBut+20
        x =self.zoom*20*self.env.jBut+20
        self.Canvas.create_rectangle(x, y, x+self.zoom*20, y+self.zoom*20, fill=self.color[1])
        self.Canvas.create_rectangle(x, y, x+self.zoom*20, y+self.zoom*20, fill=self.color[1])
    
    def move(self, direction):
        changed = 0
        cost = np.zeros(6,dtype=np.int)
        posebase = (self.env.pos_x,self.env.pos_y)
        cj = int((self.env.pos_x - 30) / (20 * self.zoom))
        li = int((self.env.pos_y - 30) / (20 * self.zoom))
        ...
        if direction == 'space':
            ...
            direction=self.env.execute_politique()
        # deplacement (-2,1)
        ...
        newcj=int((self.env.pos_x-30)/(20*self.zoom))
        newli=int((self.env.pos_y-30)/(20*self.zoom))
        self.env.posCav=(newli,newcj)
        self.window.update()
    ...
    def keyboard(self, event):
        ...
        changed, cost, posebase = self.move(key)
